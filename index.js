...
const client = new Client({
    ...
    intents: [ "GUILDS", "GUILD_MEMBERS", "GUILD_BANS", "GUILD_MESSAGE_REACTIONS", "GUILD_PRESENCES", "GUILD_MESSAGES", "DIRECT_MESSAGES" ],
});
...
client.add = async (userID, amount) => {
...
client.remove = async (userID, amount) => {
    pointsSchema.findOne({ UserID: userID }, (err, data) => {
        if (data) {
            data.Points -= amount;
            data.save();
...
client.set = async (userID, amount) => {
    pointsSchema.findOne({ UserID: userID }, (err, data) => {
        if (data) {
            data.Points = amount;
            data.save();
...
client.balance = (userID) => new Promise(async (ful) => {
